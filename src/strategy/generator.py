...
class SignalGenerator:
    ...
    def _fetch_prices(self, pair: str, size: float) -> Optional[dict]:
        ...
        try:
            ...
            if self.pricing is not None:
                # Real DEX prices from Week 2's PricingEngine
                token_in, token_out = self._pair_to_tokens(pair)
                gas_price = 1  # Arbitrum gas is cheap
                buy_quote = self.pricing.get_quote(token_in, token_out, int(size * 10**token_in.decimals), gas_price)
                sell_quote = self.pricing.get_quote(token_out, token_in, int(size * 10**token_out.decimals), gas_price)
                dex_buy = float(buy_quote.expected_output) / (size * 10**token_out.decimals)
                dex_sell = float(sell_quote.expected_output) / (size * 10**token_in.decimals)
            else:
    ...
    def _check_inventory(self, pair, direction, size, price) -> bool:
        ...
        if direction == Direction.BUY_CEX_SELL_DEX:
            return (float(self.inventory.get_available(Venue.BINANCE, quote)) >= size * price * 1.01
                    and float(self.inventory.get_available(Venue.WALLET, base)) >= size)
        else:
            return (float(self.inventory.get_available(Venue.BINANCE, base)) >= size
                    and float(self.inventory.get_available(Venue.WALLET, quote)) >= size * price * 1.01)
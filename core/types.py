...
class TokenAmount:
    ...
    def __add__(self, other: "TokenAmount") -> "TokenAmount":
        ...
        return TokenAmount(raw=self.raw + other.raw, decimals=self.decimals, symbol=self.symbol)

    def __mul__(self, factor: int | Decimal) -> "TokenAmount":
        return TokenAmount(raw=int(self.raw * factor), decimals=self.decimals, symbol=self.symbol)
...
class TransactionReceipt:
    ...
    def tx_fee(self) -> TokenAmount:
        ...
        return TokenAmount(raw=self.gas_used * self.effective_gas_price, decimals=18, symbol="ETH")
